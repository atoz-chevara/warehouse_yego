...
use App\Exports\HistoryExportFormated;
...
class ProductController extends Controller
{
    ...
    public function product_stock_history(Request $req){
        ...
        if(!empty($dl)){
            ...
            $history = $history->where('type','0')->whereRaw('DATE(datetime) = DATE(NOW())');
            return (new HistoryExportFormated($history->get()))->download('history_'.time().'.xls', \Maatwebsite\Excel\Excel::XLS);
        }
